...
with page:
    ...
    with tab_outlook:
        ...
        with wi1:
            wi_amount = st.select_slider(
                "Monthly amount", options=list(range(AMOUNT_MIN, AMOUNT_MAX + 1, AMOUNT_STEP)),
                value=monthly_amount, format_func=fmt_pkr,
                key="whatif_amount", help="What if you invested more or less per month?"
            )
        with wi2:
            wi_horizon = st.slider(
                "Horizon (years)", 5, 20, 10,
                key="whatif_horizon", help="What if you invested for a shorter or longer period?"
            )
        with wi3:
            wi_scenario = st.segmented_control(
                "Scenario", ["Bull", "Base", "Bear"], default="Base",
                required=True, key="whatif_scenario",
                help="What if the market performed better or worse?"
            )

        # Map what-if scenario to weights
        wi_weights_map = {"Bull": (1, 0, 0), "Base": (0, 1, 0), "Bear": (0, 0, 1)}
        wi_weights = wi_weights_map[wi_scenario]

        horizon_outlook = wi_horizon
        horizon_months = horizon_outlook * 12

        # Use what-if values for the Monte Carlo
        mc_result = run_monte_carlo_cached(
            wi_weights, tier, wi_amount, horizon_months,
            "bootstrap", seed=42
        )
